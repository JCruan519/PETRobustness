...
# my datasets
from .vtab import VTAB, VTAB_ADV
from .fewshot import general_dataset_few_shot
...
_FS_DATASET = ['food-101', 'oxford_pets', 'stanford_cars', 'oxford_flowers', 'fgvc_aircraft']
...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        if name == 'stanford_dogs':
            ...
            if adv_file_name is not None:
                ds = VTAB_ADV(root=root, adv_file_name=adv_file_name, get_adv_data=get_adv_data, **kwargs)
            else:
                ds = VTAB(root=root, train=is_training, **kwargs)
        elif name in _FS_DATASET:
            ds = general_dataset_few_shot(root=root, train=is_training, shot=few_shot_shot, seed=few_shot_seed, **kwargs)
        else:
