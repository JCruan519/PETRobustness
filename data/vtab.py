...
class VTAB_ADV(ImageFolder):
    def __init__(self, root, adv_file_name='test_adv_500', get_adv_data=True, transform=None, target_transform=None, mode=None,is_individual_prompt=False,**kwargs):
        self.dataset_root = root
        self.loader = default_loader
        self.target_transform = None
        self.transform = transform
        ...
        test_list_path = os.path.join(self.dataset_root, 'test_adv_500.txt')
        # test_list_path = os.path.join(self.dataset_root, 'test.txt')
        ...
        if get_adv_data:
            with open(test_list_path, 'r') as f:
                for line in f:
                    img_name = line.split(' ')[0].replace('test', adv_file_name)
                    label = int(line.split(' ')[1])
                    self.samples.append((os.path.join(root,img_name), label))
        else:
            with open(test_list_path, 'r') as f:
