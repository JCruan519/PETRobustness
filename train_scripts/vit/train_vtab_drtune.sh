t_mode=['prompt','drtune']
# seq_adapter shared_par_adapter adaptformer linear_probe prompt lora gistb gistp
GIST_FACTOR=0
GUSTB_T=0
model='vit_base_patch16_224_in21k'
DATA_ROOT_PATH=/media/ruanjiacheng/新加卷/ecodes/Prompt/data/vtab-1k
exp_tag=${GIST_FACTOR}_${GUSTB_T}
CSV_ROOT_PATH=/media/ruanjiacheng/新加卷/ecodes/Prompt/CV/GIST_ALL/outputs/${t_mode}_${exp_tag}/all_csv
CSV_PATH=${CSV_ROOT_PATH}/all_csv.csv
OUTPUT_PATH=/media/ruanjiacheng/新加卷/ecodes/Prompt/CV/GIST_ALL/outputs/${t_mode}_${exp_tag}/
master_port=$(($RANDOM + 30000))
train_py=/media/ruanjiacheng/新加卷/ecodes/Prompt/CV/GIST_ALL/train_drtune.py
...
python  -m torch.distributed.launch --nproc_per_node=$NODE_NUM  --master_port=$master_port  \
	...
	--output  ${OUTPUT_PATH}${model}/vtab/caltech101/${t_mode} \
	  --tuning-mode $t_mode --pretrained  \
    --csv_root_path ${CSV_ROOT_PATH} \
    --csv_path ${CSV_PATH}  --gist_factor ${GIST_FACTOR} --gistb_T ${GUSTB_T} \
