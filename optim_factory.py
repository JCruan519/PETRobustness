    ...
    if isinstance(model_or_params, nn.Module):
        ...
        if tuning_mode:
            for name, param in model_or_params.named_parameters():
                ...
                if 'linear_probe' in tuning_mode:
                    if "head." in name:
                        param.requires_grad = True
                if 'gistb' in tuning_mode or 'gistp' in tuning_mode or 'gistip' in tuning_mode or 'gist2' in tuning_mode:
                    if "head." in name or "gist" in name: 
                        param.requires_grad = True
                if 'prompt' in tuning_mode:
                    if "head." in name or "prompt" in name: 
                        param.requires_grad = True
                if 'seq_adapter' in tuning_mode:
                    if "head." in name or "seq_adapter" in name: 
                        param.requires_grad = True
                if 'shared_par_adapter' in tuning_mode:
                    if "head." in name or "shared_par_adapter" in name: 
                        param.requires_grad = True
                if 'adaptformer' in tuning_mode:
                    if "head." in name or "adaptformer" in name: 
                        param.requires_grad = True
                if 'lora' in tuning_mode:
                    if "head." in name or "lora" in name: 
                        param.requires_grad = True
                if 'ssf' in tuning_mode:
                    if "head." in name or "ssf" in name: 
                        param.requires_grad = True
                if 'FT' in tuning_mode:
                        param.requires_grad = True
