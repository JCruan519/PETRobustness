...
class gist_Lora(nn.Module):
    def __init__(self, dim, num_heads=8, hidden_dim=2):
        super().__init__() 
        self.mid_act = nn.GELU()
        self.linearq1 = nn.Linear(dim//num_heads, hidden_dim)
        self.linearq2 = nn.Linear(hidden_dim, dim//num_heads)
        self.lineark1 = nn.Linear(dim//num_heads, hidden_dim)
        self.lineark2 = nn.Linear(hidden_dim, dim//num_heads)
        self.scale_lora = 0.1

    def forward(self, q, k):
        q = q + self.linearq2(self.mid_act(self.linearq1(q))) * self.scale_lora
        k = k + self.lineark2(self.mid_act(self.lineark1(k))) * self.scale_lora
        return q, k
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., tuning_mode=None):
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode in ['gist_lora', 'gist_all']:
            self.gist_lora = gist_Lora(dim, num_heads, hidden_dim=2)
            print('#---------- gist_Lora wad used! ----------#')
    ...
    def forward(self, x):
        ...
        if self.tuning_mode in ['gist_lora', 'gist_all']:
            q, k = self.gist_lora(q, k)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if tuning_mode in ['gist_all', 'gist_prompt']:
            self.gist_prompt = gist_Prompt(embed_dim, prompt_len=10)
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        x = self.fc_norm(x) # self.fc_norm == Identity()
