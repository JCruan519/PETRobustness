...
class ParallelAdapter(nn.Module):
    def __init__(self, dim, hidden_dim=4, num_experts=8, drop=0., scale=0.1):
        super().__init__()
        ...
        self.parallel_fc = nn.ModuleList([nn.Linear(hidden_dim, dim) for _ in range(num_experts)])
        self.expert_weights = nn.Parameter(torch.ones(num_experts) / num_experts)
        self.drop2 = nn.Dropout(drop_probs[1])

        self.scale_adapter = scale
    ...
    def forward(self, x):
        ...
        # Use a loop to iterate over all parallel layers and compute their outputs
        parallel_outputs = [fc(x) for fc in self.parallel_fc]
        # Compute the weighted sum of the outputs of the parallel layers
        x = sum(w * o for w, o in zip(self.expert_weights, parallel_outputs))
        x = self.drop2(x)
...
class ParallelLinear(nn.Module):
    def __init__(self, in_dim, out_dim, num_experts=8):
        super().__init__()
        self.parallel_fc = nn.ModuleList([nn.Linear(in_dim, out_dim) for _ in range(num_experts)])
        self.expert_weights = nn.Parameter(torch.ones(num_experts) / num_experts)
...
class SharedInParallelOutAdapter(nn.Module):
    def __init__(self, dim, hidden_dim=4, num_experts=8, drop=0., scale=0.1):
        super().__init__()
        drop_probs = to_2tuple(drop)
        ...
        self.mid_act = nn.GELU()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.parallel_fc = ParallelLinear(hidden_dim, dim, num_experts)
        self.drop2 = nn.Dropout(drop_probs[1])
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode in ['adapter']:
            self.adapter = Adapter(dim, hidden_dim=4)
            print('#---------- Adapter wad used! ----------#')
        elif tuning_mode in ['paralleladapter']:
            self.adapter = ParallelAdapter(dim, hidden_dim=4, num_experts=8, drop=0., scale=0.1)
            print('#---------- ParallelAdapter wad used! ----------#')
        elif tuning_mode in ['sipoadapter']:
            self.adapter = SharedInParallelOutAdapter(dim, hidden_dim=2, num_experts=4, drop=0., scale=0.1)


    def forward(self, x):
        if self.tuning_mode in ['adapter', 'paralleladapter','sipoadapter']:
            x = x + self.drop_path1(self.attn(self.norm1(x))) #+ self.adapter(x)
            x = x + self.drop_path2(self.mlp(self.norm2(x))) + self.adapter(x)
        else:
