...
class base_model(nn.Module):
    def __init__(self, args, base_mdoel, ) -> None:
        super().__init__()
        self.num_classes = args.num_classes
        self.K = 768 #args.K
        self.bs = args.batch_size

        self.downstream_encoder = base_mdoel
        self.downstream_encoder.head = nn.Identity()

        self.pretrain_encoder = timm.create_model('vit_base_patch16_224_in21k', pretrained=True, tuning_mode=['None'])
        self.pretrain_encoder.head = nn.Identity()

        self.cls_head = nn.Linear(768, self.num_classes)
        self.cls_head.weight.data.normal_(mean=0.0, std=0.01)
        
        print('drtune model was created!')
        # create the queue
        self.register_buffer("down_feat_queue", torch.rand(self.K, 768, requires_grad=False))
        self.register_buffer("pre_feat_queue", torch.rand(self.K, 768, requires_grad=False))
