...
class gist_Adapter(nn.Module):
    def __init__(self, dim, hidden_dim=4):
        ...
        self.scale_adapter = 0.1 #nn.Parameter(torch.ones(1))

        # self.lpf_weight = 0.01 #nn.Parameter(torch.ones(1))
        # self.lpf = LowPassFilter(dim)

        self.hpf_weight = 0.01 #nn.Parameter(torch.ones(1))
        self.hpf = HighPassFilter(dim)

    def forward(self, x):
        return self.linear2(self.mid_act(self.linear1(x))) * self.scale_adapter + self.hpf(x) * self.hpf_weight
