...
class GIST(nn.Module):
    ...
    def forward(self, x):
        B = x.size(0)
        gist = self.prompt_dropout(self.prompt_embeddings.expand(B, -1, -1))
        x = torch.cat((
                gist,
                x
            ), dim=1)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        if mask is not None:
            ...
            # expand relative_position_bias
            mask = torch.cat((
                    torch.zeros(nW, 1, _W, device=attn.device),
                    mask), dim=1)
            mask = torch.cat((
                    torch.zeros(
                        nW, _H + 1, 1,
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x + self.gist_adapter(x)
        ...
        gist = gist.expand(num_windows, -1, -1, -1)
        gist = gist.reshape((-1, 1, C))
        x_windows = torch.cat((gist, x_windows), dim=1)
        ...
        if self.shift_size > 0:
            ...
            x = shifted_x
        x = x.view(B, H * W, C)
        x = torch.cat((gist, x), dim=1)

        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.gist_adapter(x)
...
class PatchMerging(nn.Module):
    ...
    def upsample_prompt(self, gist):
        gist = torch.cat(
                (gist, gist, gist, gist), dim=-1)
        return gist
    ...
    def forward(self, x):
        ...
        x = x[:, 1:, :]
        L = L - 1
        gist = self.upsample_prompt(gist)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #########################
        cls4att, git4att, img4att = x[:, 1:, :].max(dim=1,keepdim=True).values, x[:, 0:1, :], x[:, 1:, :].mean(dim=1,keepdim=True)
        att_loss = cal_loss_tail_att_sim(cls4att, git4att, img4att)
        
        gist, x = x[:, :1, :].squeeze(1), x[:, 1:, :]
        #########################
    ...
    def forward(self, x):
        ...
        # x = self.head(x)
        # return x
        x, gist, att_loss = self.forward_features(x)
        x, gist = self.head(x), self.head(gist)
        return x, gist, att_loss
