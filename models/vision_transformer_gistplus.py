...
def gist_transform(x, x_gist_plus):
    x_gist, x = x[:, 1:2, :], torch.cat((
                x[:, 0:1, :],
                x[:, 2:, :]
            ), dim=1)
    x_gist = x_gist + x_gist_plus
    x = torch.cat((
            x[:, :1, :],
            x_gist,
            x[:, 1:, :]
...
class Block(nn.Module):
    ...
    def forward(self, x):
        if self.tuning_mode in ['gist_adapter']:
            x_o = x
            x, x_gist_plus = self.drop_path1(self.attn(self.norm1(x))), self.gist_adapter(x[:,2:,:]).mean(dim=1, keepdim=True)
            x = gist_transform(x, x_gist_plus) + x_o
            
            x_o = x
            x, x_gist_plus = self.drop_path2(self.mlp(self.norm2(x))), self.gist_adapter(x[:,2:,:]).mean(dim=1, keepdim=True)
            x = gist_transform(x, x_gist_plus) + x_o
