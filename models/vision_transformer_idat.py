...
class gist_par_Adapter(nn.Module):
...
class gist_seq_Adapter(nn.Module):
    def __init__(self, dim, hidden_dim=4):
        super().__init__() 
        self.linear1 = nn.Linear(dim, hidden_dim)
        self.mid_act = nn.GELU()
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.tuning_mode = tuning_mode
        if tuning_mode == 'gist_seq_adapter':
            self.gist_adapter = gist_seq_Adapter(dim, hidden_dim=4)
            print('#---------- gist_seq_adapter wad used! ----------#')
        elif tuning_mode == 'gist_shared_par_adapter':
            self.gist_adapter = gist_par_Adapter(dim, hidden_dim=4)
            print('#---------- gist_shared_par_adapter wad used! ----------#')
        elif tuning_mode == 'adaptformer':
            self.gist_adapter = gist_par_Adapter(dim, hidden_dim=4)
            print('#---------- adaptformer wad used! ----------#')


    def forward(self, x):
        if self.tuning_mode == 'gist_seq_adapter':
            x = x + self.drop_path1(self.attn(self.norm1(x)))
            x = x + self.drop_path2(self.mlp(self.norm2(x)))
            x = self.gist_adapter(x)
        elif self.tuning_mode == 'gist_shared_par_adapter':
        ...
        elif self.tuning_mode == 'adaptformer':
            x = x + self.drop_path1(self.attn(self.norm1(x)))
            x = x + self.drop_path2(self.mlp(self.norm2(x))) + self.gist_adapter(x)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # weights_list = []
            # if self.tuning_mode == 'gist_seq_adapter':
            #     for i, i_block in enumerate(self.blocks):
            #         x = i_block(x)
            #         if i in [6, 7, 8, 9, 10, 11]:
            #             weights_list.append(self.blocks[i].gist_adapter.linear1.weight)
            #             weights_list.append(self.blocks[i].gist_adapter.linear2.weight.T)
            # elif self.tuning_mode == 'gist_shared_par_adapter':
            #     for i, i_block in enumerate(self.blocks):
            #         x = i_block(x)
            #         if i in [6, 7, 8, 9, 10, 11]:
    ...
    def forward(self, x):
        ...
        x, weights_list = self.forward_features(x)
