...
#         super().__init__() 
#         self.linearin_1 = nn.Linear(dim, hidden_dim)
#         self.linearin_2 = nn.Linear(dim, hidden_dim)
#         self.linearin_3 = nn.Linear(dim, hidden_dim)
#         self.linearin_4 = nn.Linear(dim, hidden_dim)
#         self.mid_act = nn.GELU()
#         self.linearout = nn.Linear(hidden_dim*4, dim)
#         self.scale_adapter = 0.1

#     def forward(self, x):
#         x1 = self.linearin_1(x)
#         x2 = self.linearin_2(x)
#         x3 = self.linearin_3(x)
#         x4 = self.linearin_4(x)
...
#         self.in_conv1 = nn.Conv1d(dim//4, dim//4, kernel_size=3, stride=1,
#                                   padding=1, dilation=1, groups=dim//4)
#         self.in_conv2 = nn.Conv1d(dim//4, dim//4, kernel_size=3, stride=1,
#                                   padding=2, dilation=2, groups=dim//4)
#         self.in_conv3 = nn.Conv1d(dim//4, dim//4, kernel_size=3, stride=1,
#                                   padding=5, dilation=5, groups=dim//4)
#         self.in_conv4 = nn.Conv1d(dim//4, dim//4, kernel_size=3, stride=1,
#                                   padding=7, dilation=7, groups=dim//4)
...
#     def forward(self, x):
#         x = torch.chunk(x.permute(0,2,1), 4, dim=1)
#         x0 = self.in_conv1(x[0])
#         x1 = self.in_conv2(x[1])
#         x2 = self.in_conv3(x[2])
#         x3 = self.in_conv4(x[3])
...
class GIST(nn.Module):
    def __init__(self, model_dim, prompt_len=1, prompt_dim=768, dropout_ratio=0.1):
        super().__init__()
...
#                 x[:, 1:, :]
#             ), dim=1)
#         gist = self.transform_block(gist)
#         x = torch.cat((
#                 x[:, :1, :],
#                 gist,
#                 x[:, 1:, :]
...
class Instance_GIST(nn.Module):
    def __init__(self, model_dim, prompt_len=1, dropout_ratio=0.1):
        ...
        self.transform_block = nn.Sequential(
            nn.LayerNorm(model_dim),
            nn.Conv1d(197,prompt_len,1),
            nn.GELU(),
        )


    def forward(self, x):
        gist = self.transform_block(x)
        x = torch.cat((
                x[:, :1, :],
                gist,
                x[:, 1:, :]
