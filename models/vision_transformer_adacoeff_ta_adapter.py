...
#         self.init_weight = nn.Parameter(torch.Tensor(1, 1, 768))
#         nn.init.ones_(self.init_weight)

#     def forward(self, x):
#         # x = self.linear2(self.mid_act(self.linear1(x)))
#         e_weight = self.linear2(self.mid_act(self.linear1(self.init_weight)))
#         x = x * e_weight
...
#     def forward(self, x):
#         noise = torch.rand_like(x) * 0.001
#         return (self.linear2(self.mid_act(self.linear1(x))) + noise) * self.scale_adapter
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if tuning_mode in ['gist_adapter']:
            self.gist_adapter = gist_Adapter(dim, hidden_dim=4)
